...
if(!empty($_GET['action'])&&($_GET['action']=='delete')){
    if(!empty($_GET['id'])&&($_GET['id']>0)){
        $mysqli->query("DELETE FROM adherent WHERE id=".$_GET['id']);
    }
    ...
    $result=$mysqli->query("SELECT count(*) as cnt FROM adherent")->fetch_assoc();
    $count = $result['cnt'];
    ...
    $offset=$nb*($pg-1);
    $where=!empty($search)? " WHERE nom LIKE '%" . $search . "%' OR prenom LIKE '%" . $search . "%' ": '';

    $query="SELECT * FROM adherent ".$where." ORDER BY ".$sort." ".$dir." LIMIT ".$nb ." OFFSET ".$offset;
...
?></span><img class="border rounded-circle img-profile" src="assets/img/avatars/avatar<?php  echo $_SESSION['user_id'] ; ?>.jpeg"></a>
                ...
                <div style="height: 50px;margin-bottom: 20px ;"> <form action= "liste_adhérents.php" method="GET" class="d-none d-sm-inline-block me-auto ms-md-3 my-2 my-md-0 mw-100 navbar-search">
                ...
                <div class="container-fluid">
                    <h3 class="text-dark mb-4">Listes adhérents</h3>
                    <div class="card shadow">
                        <div class="card-header py-3">
                            <p class="text-primary m-0 fw-bold">Adhérents Info</p>
                        </div>
                        <div class="card-body">
                            <div class="row">
...
<option value="100" <?=$nb==100?'selected':''?>>100</option>
                                ...
                                <div class="col-md-6" style="width: 455.5px;"> <div class="text-md-end dataTables_filter" id="dataTable_filter"><a class="btn btn-primary" role="button" href="adhérent.php?id=0" style="border-radius: 17px;">Ajouter</a></td></div>
                </div>
                            </div><div id="dataTable" class="table-responsive table mt-2" role="grid" aria-describedby="dataTable_info">
    <table id="dataTable" class="table my-0">
        <thead>
            <tr>
                <th style="width: 170px;"><a href="liste_adhérents.php?sort=nom&nb=<?=$nb?>&pg=<?=$pg?>&dir=<?=$dir=='ASC'?'DESC':'ASC'?>">Nom</a><?=$sort=='nom'?$dir=='ASC'?'<i class="fas fa-sort-alpha-down"></i>':'<i class="fas fa-sort-alpha-down-alt"></i>':''?></th>
                <th style="width: 0px;"><a href="liste_adhérents.php?sort=prenom&nb=<?=$nb?>&pg=<?=$pg?>&dir=<?=$dir=='ASC'?'DESC':'ASC'?>">Prénom</a><?=$sort=='prenom'?$dir=='ASC'?'<i class="fas fa-sort-alpha-down"></i>':'<i class="fas fa-sort-alpha-down-alt"></i>':''?></th>
                <th style="width: 70px;"><a href="liste_adhérents.php?sort=sexe&nb=<?=$nb?>&pg=<?=$pg?>&dir=<?=$dir=='ASC'?'DESC':'ASC'?>">Sexe</a><?=$sort=='sexe'?$dir=='ASC'?'<i class="fas fa-sort-alpha-down"></i>':'<i class="fas fa-sort-alpha-down-alt"></i>':''?></th>
                <th style="width: 10px;"><a href="liste_adhérents.php?sort=birthday&nb=<?=$nb?>&pg=<?=$pg?>&dir=<?=$dir=='ASC'?'DESC':'ASC'?>">Birthday</a><?=$sort=='birthday'?$dir=='ASC'?'<i class="fas fa-sort-alpha-down"></i>':'<i class="fas fa-sort-alpha-down-alt"></i>':''?></th>
                <th style="width: 10px;"><a href="liste_adhérents.php?sort=date_inscription&nb=<?=$nb?>&pg=<?=$pg?>&dir=<?=$dir=='ASC'?'DESC':'ASC'?>">Inscription</a><?=$sort=='date_inscription'?$dir=='ASC'?'<i class="fas fa-sort-alpha-down"></i>':'<i class="fas fa-sort-alpha-down-alt"></i>':''?></th>
                <th style="width: 10px;"><a href="liste_adhérents.php?sort=telephone&nb=<?=$nb?>&pg=<?=$pg?>&dir=<?=$dir=='ASC'?'DESC':'ASC'?>">Téléphone</a><?=$sort=='telephone'?$dir=='ASC'?'<i class="fas fa-sort-alpha-down"></i>':'<i class="fas fa-sort-alpha-down-alt"></i>':''?></th>
...
<tr>
 <td><img class="rounded-circle me-2" width="45" height="45" src="assets/img/avatars/avatar<?=$row['id']?>.jpeg">&nbsp;<?=$row['nom']?> </td>
  <td><?=$row['prenom']?></td>
  <td><?=$row['sexe']?></td>
 <td><?=$row['birthday']?></td>
 <td><?=$row['date_inscription']?></td>
  <td><?=$row['telephone']?></td>
<td><a class="btn btn-primary" role="button" href="adhérent.php?id=<?=$row['id']?>" style="border-radius: 17px;">Modifier</a>&nbsp&nbsp&nbsp&nbsp
...
<a class="btn btn-primary" role="button" onClick="confirmation();" href="liste_adhérents.php?action=delete&id=<?=$row['id']?>&nb=<?=$nb?>&pg=<?=$pg?>"
...
?>
        ...
        <tfoot>
            <tr>
                ...
                <td><strong><strong>Inscription</strong><br /></strong></td>
                <td><strong><strong>Téléphone</strong><br /></strong></td>
                <td><strong><strong>Modifier</strong><br /></strong></td>
            </tr>
        </tfoot>
    </table>
</div>
